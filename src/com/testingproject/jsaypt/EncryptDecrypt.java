...
public class EncryptDecrypt {
	 ...
	 * and invoke two methods for encryption and decryption job. After
	 * successful job the constructor puts the decrypted password in variable to
	 * be retrieved by calling class.
	 * 
	 * 
	 * @param pPropertyFileName
	 *            /Name of the properties file that contains the password
	 * @param pUserPasswordKey
	 *            /Left hand side of the password property as key.
	 * @param pIsPasswordEncryptedKey
	 *            /Key in the properties file that will tell us if the password
	 ...
	 * first check if the password is already encrypted or not. If not then only
	...
	private void encryptPropertyValue() throws ConfigurationException {
		...
		if (isEncrypted.equals("false")) {
			...
			// This is a required password for Jasypt. You will have to use the
			// same password to
			// retrieve decrypted password later. T
			// This password is not the password we are trying to encrypt taken
